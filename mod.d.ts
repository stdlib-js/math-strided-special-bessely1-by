...
/**
* Interface describing `bessely1By`.
*/
interface Routine {
	/**
	...
	*/
	...
	/**
	* Computes the Bessel function of the second kind of order one for each element retrieved from an input strided array `x` via a callback function and assigns each result to an element in an output strided array `y` using alternative indexing semantics.
	*
	* @param N - number of indexed elements
	* @param x - input array
	* @param strideX - `x` stride length
...
* // y => [ -Infinity, ~-0.781, ~-6.459, ~-2.704, ~-1.471 ]
*/
declare var bessely1By: Routine;
